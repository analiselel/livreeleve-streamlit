...
devolucoes = pd.read_excel(arquivos_devolucoes)

# Definir as colunas que serão derretidas (melted)
colunas_melt = ['Total de Reversas', 'Trocas', 'Devoluções', 'Valor das Trocas', 
                'Valor das Devoluções', 'Tamanho - Ficou Grande', 'Tamanho - Ficou Pequeno',
                'Não gostei do produto', 'Transparência', 'Não vestiu bem', 
                'Defeito - Problemas na costura', 'Marketplace', 
                'Produto errado - Comprei um produto e recebi outro', 
...
def dashboard_devolucoes():
   ...
   # Lógica para filtrar por 'produto1'
    ...
    # Derreter as colunas
    devolucoes_melted = filtro_prod1.melt(value_vars=colunas_melt,
                                        var_name='Motivo', 
                                        value_name='Quantidade')
    
   # Calcular a soma das quantidades por motivo
    soma_por_motivo = devolucoes_melted.groupby('Motivo')['Quantidade'].sum().reset_index()

    # Adicionar uma coluna 'Total' que seja a soma dos valores das colunas derretidas
    soma_por_motivo['Total'] = soma_por_motivo['Quantidade']

    # Exibir os resultados
    st.write(soma_por_motivo[['Motivo', 'Total']].to_html(index=False), unsafe_allow_html=True, width=1000, height=600)