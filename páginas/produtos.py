...
def dashboard_produtos():
    ...
    st.markdown("**Produtos comprados em conjunto**")
    ...
    filtro_prod1['Data'] = filtro_prod1['Creation Date'].dt.date

    # Agrupar os dados por data e calcular a média do 'SKU Selling Price'
    dados_agrupados = filtro_prod1.groupby('Data').agg({'SKU Selling Price': 'mean', 'Order': 'count'}).reset_index()

    # Renomear as colunas
    dados_agrupados.rename(columns={"SKU Selling Price": "Ticket Médio", "Order": "Número de Vendas"}, inplace=True)

    # Criação do gráfico de linha com Plotly Express
    fig = px.line(dados_agrupados, x='Data', y='Ticket Médio', title='Ticket Médio ao longo do período')

    # Adicionando anotações para cada ponto
    prev_y = None
    for i in range(len(dados_agrupados)):
        if prev_y is None or dados_agrupados['Ticket Médio'][i] != prev_y:
            text = f"<b>R${dados_agrupados['Ticket Médio'][i]:.2f}</b>"
            if dados_agrupados['Ticket Médio'][i] == dados_agrupados['Ticket Médio'].max():
            ...
            text = ""
            y_shift = -20
        fig.add_annotation(x=dados_agrupados['Data'][i], y=dados_agrupados['Ticket Médio'][i],
                        text=text, showarrow=False, xshift=5, yshift=y_shift)
        prev_y = dados_agrupados['Ticket Médio'][i]
    ...
    st.markdown("**Compras por região**")
